...
# saisir action
class SaisirSyntheseAction(models.Model):
    _name="saisir.synthese.action"
    ...
    def _check_unique_record(self):
        for record in self:
            ...
            val_anne = self.env['saisir.synthese.action'].search(
    ...
    saisir_action_id=fields.Many2one("saisir.action")
    ...
    def write(self, vals):
        ...
        res= super(SaisirSyntheseAction, self).write(vals)
        if res:
            for line in user_admin:
                subject = "DIGISTRAT : Modification d'une saisie de synthèse d'action"
